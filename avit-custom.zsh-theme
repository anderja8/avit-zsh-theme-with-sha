...
# settings
# Previous _current_dir variable (showed only the last 3 directories typeset +H _current_dir="%{$fg_bold[blue]%}%3~%{$reset_color%} "
...
RPROMPT='$(vi_mode_prompt_info)%{$(echotc UP 1)%}$(git_short_sha_info) $(_git_time_since_commit) $(git_prompt_status) ${_return_status}%{$(echotc DO 1)%}'
...
function git_short_sha_info() {
  if [ -d .git ]; then
    echo "%{$fg_bold[blue]%}[$(git rev-parse --short HEAD)]%{$reset_color%}"
  fi;
}
